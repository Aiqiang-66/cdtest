...
OUT = Path(r'd:\python\dmx\cdtest\docs\听书测试物料\mp3\0724_XR3_6lang')
...
def main():
    # 6语言配置：文件名、章节正则、lang、voice、中文名
    configs = [
        ('XR3 - EN.txt', r'=== Chapter\s+\d+',       1, 'English_female',    '英语'),
        ('XR3 - DE.txt', r'=== Kapitel\s+\d+',       1, 'German_female',    '德语'),
        ('XR3 - FR.txt', r'=== Chapitre\s+\d+',      6, 'French_female',    '法语'),
        ('XR3 - SP.txt', r'=== Capítulo\s+\d+',      4, 'Spanish_female',   '西语'),
        ('XR3 - PT.txt', r'=== Capítulo\s+\d+',      5, 'Portuguese_female','葡语'),
        ('XR3 - RU.txt', r'=== Глава\s+\d+',         1, 'Russian_female',   '俄语'),
    ]
    ...
    print(f"\n{'='*60}")
    print("汇总：XR3小说第1章，6语言生成耗时对比")
    print(f"{'='*60}")
    print(f"{'语言':22s} {'taskId':>8s} {'字符':>8s} {'耗时':>8s} {'mp3大小':>10s} {'字幕':>5s}")
    print("-" * 70)
    for r in results:
        label = r['label']
        tid = str(r['taskId'])
        ...
        if r['ok']:
            print(f"{label:22s} {tid:>8s} {chars:>8,} {r['elapsed']:>7.1f}s {r['mp3_size']:>10,} {r['subs']:>5}")
        else:
            print(f"{label:22s} {tid:>8s} {chars:>8,} {r['elapsed']:>7.1f}s {'FAIL':>10s}")
