...
# 2. 检查最近的扩语种任务
print("\n2. 最近的扩语种任务（SubTaskType=4）：")
cursor.execute("""
    SELECT Id, SubTaskType, Status, CreateTime, Creator 
...
""")
for row in cursor.fetchall():
    print(f"  ID={row[0]}, Type={row[1]}, Status={row[2]}, Time={row[3]}, Creator={row[4]}")
...
print("\n4. 最近的子任务：")
cursor.execute("""
    SELECT Id, TaskId, Status, CreateTime 
    FROM AIMixedClipSubTask 
    ORDER BY CreateTime DESC 
    LIMIT 5
""")
for row in cursor.fetchall():
    print(f"  ID={row[0]}, TaskId={row[1]}, Status={row[2]}, Time={row[3]}")
